...
public class StateTransitionQueryFactory extends AbstractPatternQueryFactory {
	 ...
	 */
	@Override
	protected PatternQuery generateQuery(final AbstractBPMNElement element, final AbstractBPMNElement catchingMonitorableElement, final PatternQuery parentQuery) {
		...
		if (this.patternQueryGenerator.getQueryForElement(element) != null) {
			...
			query = new PatternQuery(this.generateQueryName("StateTransition"), queryString, QueryTypeEnum.LIVE, PatternQueryType.STATETRANSITION, Arrays.asList(element));
			this.addQueryRelationship(parentQuery, query);
	 ...
	 * @return
	 */
	private String generateQueryString(final AbstractBPMNElement element, final AbstractBPMNElement catchingMonitorableElement) {
		// not all monitoring points of an element must be present
		// In general Query-Cycle: (Enable->Begin->Terminate) OR Skip, Disrupt
		final MonitoringPoint initializeMonitoringPoint = element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.initialize);
		final MonitoringPoint enableMonitoringPoint = element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.enable);
		final MonitoringPoint beginMonitoringPoint = element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.begin);
		final MonitoringPoint terminateMonitoringPoint = element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.terminate);
		final MonitoringPoint skipMonitoringPoint = element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.skip);
		element.getMonitoringPointByStateTransitionType(MonitoringPointStateTransition.disrupt);

		final List<MonitoringPoint> regularMonitoringPoints = Arrays.asList(initializeMonitoringPoint, enableMonitoringPoint, beginMonitoringPoint, terminateMonitoringPoint);
		...
		final boolean elementHasSkipMonitoringPoint = skipMonitoringPoint != null && skipMonitoringPoint.getEventType() != null;
