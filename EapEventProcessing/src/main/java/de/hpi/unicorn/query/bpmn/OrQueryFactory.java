...
public class OrQueryFactory extends AbstractPatternQueryFactory {
	...
	protected PatternQuery generateQuery(final AbstractBPMNElement element, final AbstractBPMNElement catchingMonitorableElement, final PatternQuery parentQuery) throws QueryGenerationException {
		if (element instanceof Component) {
			...
			if (!this.allPathesContainMonitoringPoints(component)) {
				throw new QueryGenerationException("Query creation failed for: " + element + " Reason: On all pathes of an exclusive component should be a monitorable element!");
			}
			// Component sollte mehrere Polygone beinhalten
			// Operator: OR
			final PatternQuery query = new PatternQuery(this.generateQueryName("XOR"), null, QueryTypeEnum.LIVE, PatternQueryType.XOR, this.orderElements(component));

			final String queryString = this.generateQueryString(component, EsperPatternOperators.XOR, catchingMonitorableElement, query);
	...
	private boolean allPathesContainMonitoringPoints(final Component component) {
		...
		for (final AbstractBPMNElement child : this.patternQueryGenerator.getRPSTTree().getProcessDecompositionTree().getChildren(component)) {
