...
public class TimerQueryFactory extends AbstractPatternQueryFactory {
	...
	private PatternQuery generateAttachedTimerQuery(final AbstractBPMNElement element, final AbstractBPMNElement catchingMonitorableElement, final PatternQuery parentQuery) {
		final StringBuilder sequencePatternQueryString = new StringBuilder();
		sequencePatternQueryString.append("SELECT * FROM PATTERN [(");

		final PatternQuery query = new PatternQuery(this.generateQueryName("Timer"), null, QueryTypeEnum.LIVE, PatternQueryType.TIMER, Arrays.asList(element));
		this.addQueryRelationship(parentQuery, query);
		// TODO: Hier die Timer-Query
		final PatternQuery subQuery = new StateTransitionQueryFactory(this.patternQueryGenerator).generateQuery(element, catchingMonitorableElement, query);
		...
		final EapEventType boundaryTimerEventType = this.addPatternEventTypeToEsper(timerEventTypeName + "BoundaryTimer");
		...
		sequencePatternQueryString.append(" WHERE de.hpi.unicorn.esper.EapUtils.isIntersectionNotEmpty({S0.ProcessInstances, S1.ProcessInstances})");
	...
	private PatternQuery generateInterMediateTimerQuery(final AbstractBPMNElement element, final AbstractBPMNElement catchingMonitorableElement, final PatternQuery parentQuery) {
		...
		if (intermediateEvent.getIntermediateEventType().equals(BPMNEventType.Timer)) {
			...
			timerQuery = new PatternQuery(this.generateQueryName("Timer"), null, QueryTypeEnum.LIVE, PatternQueryType.TIMER, Arrays.asList(element));
			...
			final List<PatternQuery> predecessorQueries = new ArrayList<PatternQuery>(this.findStateTransitionQueriesForElements(monitorablePredecessors));
			...
			if (!predecessorQueries.isEmpty()) {
				...
				final EapEventType interMediateTimerEventType = this.addPatternEventTypeToEsper(intermediateEvent.getName() + "Timer");
				...
				sequencePatternQueryString.append(" WHERE de.hpi.unicorn.esper.EapUtils.isIntersectionNotEmpty({S1.ProcessInstances, S2.ProcessInstances})");
				...
				for (final PatternQuery predecessorQuery : predecessorQueries) {
					...
					final EapEventType timmerTriggerEventType = timerTrigger.getMonitoringPoints().get(0).getEventType();
	...
	private PatternQuery createPredecessorORQuery(final List<PatternQuery> predecessorQueries) {
		final PatternQuery predecessorOrQuery = new PatternQuery(this.generateQueryName("TimerPredecessor"), null, QueryTypeEnum.LIVE, PatternQueryType.XOR, new ArrayList<AbstractBPMNElement>());
		final StringBuilder sequencePatternQueryString = new StringBuilder();
	 ...
	 */
	private void getNearestMonitorablePredecessors(final AbstractBPMNElement element, final Set<AbstractBPMNElement> monitorablePredecessors, final Set<AbstractBPMNElement> visitedpredecessors) {
