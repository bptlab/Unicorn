		...
		if (element instanceof Component) {
			...
			if (component.hasMonitoringPointsWithEventType()) {
				switch (component.getType()) {
					case AND:
						query = new AndQueryFactory(this.patternQueryGenerator).generateQuery(component, catchingMonitorableElement, parentQuery);
						break;
					case LOOP:
						query = new LoopQueryFactory(this.patternQueryGenerator).generateQuery(component, catchingMonitorableElement, parentQuery);
						break;
					case SEQUENCE:
						query = new SequenceQueryFactory(this.patternQueryGenerator).generateQuery(component, catchingMonitorableElement, parentQuery);
						break;
					case XOR:
						query = new OrQueryFactory(this.patternQueryGenerator).generateQuery(component, catchingMonitorableElement, parentQuery);
						break;
					case SUBPROCESS:
						// TODO: Kann SubProcess noch ein externes Catching-Event
						// haben?
						query = new SubProcessQueryFactory(this.patternQueryGenerator).generateQuery(component, catchingMonitorableElement, parentQuery);
						break;
					default:
