...
public class TransformationRuleLogic implements Serializable {
	 ...
	 *
	 * @param transformationRule the {@link TransformationRule}
	...
	public static void checkForValidity(final TransformationRule transformationRule) {
		...
		final ArrayList<String> attributesOfTargetEventType = transformationRule.getEventType().getAttributeExpressions();
		...
		for (final String attribute : attributesFromRule) {
			if (!attributesOfTargetEventType.contains(attribute)) {
				throw new EPException("Error in transformation rule: The attribute '" + attribute + "' does not exist in target event type '" + transformationRule.getEventType().getTypeName() + "'.");
			}
		}
	...
	public void checkForValidity(final EapEventType selectedEventType, final String transformationRuleName, final Map<String, String> attributeIdentifiersAndExpressions, final Map<String, ExternalKnowledgeExpressionSet> attributeIdentifiersWithExternalKnowledge, final TransformationPatternTree patternTree) throws RuntimeException {// TODO:
		...
		for (final FilterExpressionElement element : patternTree.getFilterExpressionElements()) {
			if ((element.getValue() == FilterExpressionOperatorEnum.IN || element.getValue() == FilterExpressionOperatorEnum.NOT_IN)) {
				...
				if (element.getLeftHandSideExpression() == null || element.getLeftHandSideExpression().isEmpty() || element.getRightHandSideExpression() == null || element.getRightHandSideExpression().isEmpty()) {
		...
		while (iterator.hasNext()) {
			...
			if (attributeIdentifiersWithExternalKnowledge.get(attributeIdentifier) != null) {
				for (final ExternalKnowledgeExpression expression : attributeIdentifiersWithExternalKnowledge.get(attributeIdentifier).getExternalKnowledgeExpressions()) {
	 ...
	 */
	public TransformationRule createTransformationRule(final EapEventType selectedEventType, final String transformationRuleName, final TransformationPatternTree patternTree, final Map<String, String> attributeIdentifiersAndExpressions, final Map<String, ExternalKnowledgeExpressionSet> attributeIdentifiersWithExternalKnowledge) {
