...
/**
 * tree component for visualization of hierarchical elements one node can be
 * selected at a time
 * 
 * @param <T>
 *            the type of nodes to be stored in the tree
 */
public class SelectTree<T> extends LabelTree<T> {

	private static final long serialVersionUID = 1L;
	private IModel<T> selectedElement;
	private final SelectTree<T> selectTree;
	...
	protected Component newContentComponent(final String id, final IModel<T> model) {
		return new TreeLinkLabel<T>(id, this, model) {
			...
			protected void onClick(final AjaxRequestTarget target) {
				SelectTree.this.selectTree.select(this.getModelObject(), SelectTree.this.selectTree, target);
				if (SelectTree.this.selectedElement != null) {
					// System.out.println("Selected element: " +
					// selectedElement.getObject());
				} else {
			...
			protected boolean isSelected() {
				return SelectTree.this.selectTree.isSelected(this.getModelObject());
			}
		};
	}
