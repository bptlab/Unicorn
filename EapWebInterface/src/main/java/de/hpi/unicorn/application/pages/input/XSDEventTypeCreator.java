...
public class XSDEventTypeCreator extends AbstractEapPage {
	...
	public XSDEventTypeCreator(final PageParameters parameters) throws XMLParsingException {
		...
		this.timestampDropDownChoice = new DropDownChoice<String>("timestampDropDownChoice", new PropertyModel<String>(this, "timestampXPath"), this.leafPathes);
		this.timestampDropDownChoice.add(new AjaxFormComponentUpdatingBehavior("onchange") {
			...
			protected void onUpdate(final AjaxRequestTarget target) {
				...
				XSDEventTypeCreator.this.timestamp = XSDEventTypeCreator.this.eventTypeAttributesTree.getAttributeByXPath(XSDEventTypeCreator.this.timestampXPath);
	...
	protected void renderTree() {
		this.tree = new MultiSelectTree<TypeTreeNode>("eventTypeTree", new AttributeTreeProvider(this.eventTypeAttributesTree.getRoots()), new AttributeTreeExpansionModel()) {

			@Override
			protected void toggle(final TypeTreeNode element, final AbstractTree<TypeTreeNode> tree, final AjaxRequestTarget target) {
				if (XSDEventTypeCreator.this.eventTypeUsingImportTime || !element.equals(XSDEventTypeCreator.this.timestamp)) {
	...
	private void buildButtons() {
		...
		final AjaxButton selectAllButton = new AjaxButton("selectAllButton") {
			...
			public void onSubmit(final AjaxRequestTarget target, final Form<?> form) {
				XSDEventTypeCreator.this.tree.getSelectedElements().addAll(XSDEventTypeCreator.this.eventTypeAttributesTree.getAttributes());
				target.add(XSDEventTypeCreator.this.tree);
			}
		};
		...
		final Button confirmButton = new Button("confirmButton") {
			...
			public void onSubmit() {
				...
				try {
					XSDEventTypeCreator.this.importedEventType.setTypeName(XSDEventTypeCreator.this.eventTypeNameTextField.getModelObject());
					if (XSDEventTypeCreator.this.eventTypeUsingImportTime) {
						XSDEventTypeCreator.this.importedEventType.setTimestampName(AbstractXMLParser.GENERATED_TIMESTAMP_COLUMN_NAME);
					} else {
						...
						if (!XSDEventTypeCreator.this.timestamp.hasParent()) {
							XSDEventTypeCreator.this.eventTypeAttributesTree.removeRoot(XSDEventTypeCreator.this.timestamp);
							XSDEventTypeCreator.this.importedEventType.setValueTypeTree(XSDEventTypeCreator.this.eventTypeAttributesTree);
						}
					...
					if (EapEventType.findByTypeName(XSDEventTypeCreator.this.importedEventType.getTypeName()) != null) {
						XSDEventTypeCreator.this.xsdEventTypeCreator.getFeedbackPanel().error("Event type " + XSDEventTypeCreator.this.importedEventType.getTypeName() + " already exists");
						return;
					}
					...
					pageParameters.add("successFeedback", "Event type " + XSDEventTypeCreator.this.importedEventType.getTypeName() + " has been created");
					this.setResponsePage(MainPage.class, pageParameters);
				} catch (final RuntimeException e) {
					e.printStackTrace();
					XSDEventTypeCreator.this.xsdEventTypeCreator.getFeedbackPanel().error("Event type has not been created. See output console for more details.");
				}
			}
		};
