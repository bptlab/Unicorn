...
public class MapTreePersistenceTest implements PersistenceTest {
	...
	public void testMapElementStoreAndRetrieve() {
		EventTransformationElement<String, String> mapElement = new EventTransformationElement<String, String>("Key", "Value");
		mapElement.save();
		assertTrue("Value should be 1, but was " + EventTransformationElement.findAll().size(), EventTransformationElement.findAll().size() == 1);
		EventTransformationElement.removeAll();
		assertTrue("Value should be 0, but was " + EventTransformationElement.findAll().size(), EventTransformationElement.findAll().size() == 0);
	}
	...
	public void testStoreAndRetrieve() {
		storeExampleMapTree();
		assertTrue("Value should be 1, but was " + TransformationTree.findAll().size(), TransformationTree.findAll().size() == 1);
		...
		assertTrue("Value should be 0, but was " + TransformationTree.findAll().size(), TransformationTree.findAll().size() == 0);
	}
	...
	public void testSaveDifferentKindsOfSushiMaptree() {
		...
		TransformationTree<String, Integer> testMapTree2 = new TransformationTree<String, Integer>(rootElement1Key, rootElement1Value);
		...
		TransformationTree<String, Date> testMapTree3 = new TransformationTree<String, Date>(rootElement1Key2, rootElement1Value2);
		testMapTree3.save();
		allSushiMapTrees = TransformationTree.findAll();
	...
	public void testObjectValueType() {
		...
		TransformationTree<String, Serializable> testMapTree = new TransformationTree<String, Serializable>(rootElement1Key, rootElement1Value);
